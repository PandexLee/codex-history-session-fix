param(
  ...
  [string]$OutputRoot = (Join-Path $env:TEMP 'codex-history-sidebar-msix-patch')
)

<#
.SYNOPSIS
  Patch the installed Windows Codex Desktop MSIX package to avoid one history sidebar refresh bug.

.DESCRIPTION
  Copies the installed OpenAI.Codex package to a temporary work directory, extracts app.asar,
  applies scripts/patch-history-sidebar.cjs, and validates the patched asset. Without -DryRun,
  it repacks and signs a patched MSIX. With -Install, it removes the installed package and
  installs the patched package with Add-AppxPackage -AllowUnsigned.

.NOTES
  Platform: Windows PowerShell / PowerShell.
  Mutates system: only when -Install is used. -DryRun does not install a package.
  Prerequisites: Node.js, npx, Windows SDK makeappx.exe and signtool.exe.
  -InstallPrerequisites may download Microsoft Windows SDK BuildTools from NuGet.
...
$LogPrefix = '[codex-history-sidebar-msix]'
...
$workRoot = Join-Path $OutputRoot "history-sidebar-$($pkg.Version)-$stamp"
$workPackageRoot = Join-Path $workRoot 'package'
$asarDir = Join-Path $workRoot 'app-asar'
$npxCache = Join-Path $workRoot 'npm-cache'
$msixPath = Join-Path $OutputRoot "OpenAI.Codex_$($pkg.Version)_history-sidebar-patched.msix"
$patcher = Join-Path $PSScriptRoot 'patch-history-sidebar.cjs'
$installedSuccessfully = $false

if (-not (Test-Path -LiteralPath $patcher -PathType Leaf)) {
  Fail "history sidebar patcher not found: $patcher"
}
...
try {
  ...
  Write-Log 'patching history sidebar thread list refresh'
  $patchResult = & node $patcher $asarDir
  if ($LASTEXITCODE -ne 0) {
    Fail "history sidebar patch failed with exit code $LASTEXITCODE"
  }
  Write-Log "history sidebar patch result: $patchResult"
  $historyFile = Get-ChildItem -LiteralPath (Join-Path $asarDir 'webview\assets') -Filter 'thread-context-inputs-*.js' -File | Select-Object -First 1
  if (-not $historyFile) {
    Fail 'thread-context-inputs asset missing after extraction'
  }
  & node --check $historyFile.FullName
  if ($LASTEXITCODE -ne 0) {
    Fail "node syntax check failed for $($historyFile.FullName)"
  }
